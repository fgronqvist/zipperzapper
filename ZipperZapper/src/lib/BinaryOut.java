...
/**
 * ****************************************************************************
 * Compilation: javac BinaryOut.java Execution: java BinaryOut Dependencies:
 * none
 *
 * Write binary data to an output stream, either one 1-bit boolean, one 8-bit
 * char, one 32-bit int, one 64-bit double, one 32-bit float, or one 64-bit long
 * at a time. The output stream can be standard output, a file, an OutputStream
 * or a Socket.
 *
...
public final class BinaryOut {
    ...
    /**
     * Flushes the binary output stream, padding 0s if number of bits written so
     * far is not a multiple of 8.
     */
    public void flush() {
        clearBuffer();
        try {
    ...
    /**
     * Flushes and closes the binary output stream. Once it is closed, bits can
